...
from schemas.ingrediente import IngredienteCreate, IngredienteUpdate, Ingrediente
...
def aggiorna_ingrediente(ingrediente_id: int, ingrediente: IngredienteUpdate,
                         db: Session = Depends(get_db)):
    ingr = crud_ingrediente.update_ingrediente(db, ingrediente_id, ingrediente)
    if not ingr:
        raise HTTPException(status_code=404, detail="Ingrediente non trovato")
