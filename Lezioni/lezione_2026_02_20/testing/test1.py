...
def test_somma(input1, input2, corretto):
    ...
    assert risultato == corretto, f'La funzione "somma" non ha dato il risultato sperato ({input1}+{input2}={corretto} | {risultato})'
...
def test_area(base: float, altezza: float, corretto: float):
    ...
    assert risultato == corretto, f"L'area corretta era ({base} * {altezza} / 2 = {corretto} | {risultato})"

def test_area_multipla():
    test_area(10, 8, 40)
